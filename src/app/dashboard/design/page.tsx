...
export default async function DesignPage() {
  ...
  const { data: profile } = await supabase
    .from("profiles")
    .select(
      "username, display_name, bio, theme, font, plan, avatar_url, avatar_shape, avatar_size, banner_url, banner_size, banner_fade, custom_bg_color, custom_card_color, custom_text_color",
    )
    .eq("id", user.id)
    .single();
